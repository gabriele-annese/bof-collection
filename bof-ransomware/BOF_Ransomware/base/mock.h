...
namespace bof {
    ...
    namespace output {
        ...
        struct RuturnData {
			std::vector<bof::output::OutputEntry> outputs;
            int returnVal;
        };
     ...
     */
    template <typename... T>
    struct bof::output::RuturnData runMocked(int (*entry)(char*, int), T &&...v) {
        struct bof::output::RuturnData resultData;
        ...
        resultData.returnVal = entry(args.get(), args.size());
        // Return the stored outputs
        resultData.outputs = bof::output::getOutputs();
        return resultData;
    }
}